...
//log that the module is loaded
    console.log(` :: ⬜️ Module: Interview Chat Enchanced, Invalid `)
//voice state update event to check joining/leaving channels
  ...
  if (!oldState.channelID && newState.channelID) {
    if(newState.channelID !== config.INT) return;  //if its not the jointocreatechannel skip
    jointocreatechannel(newState);   //load the function
  }
  // LEFT V12
  if (oldState.channelID && !newState.channelID) {
            //get the jointocreatechannel id from the map
          if (intmap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
            //fetch it from the guild
            var vc = oldState.guild.channels.cache.get(intmap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`));
            //if the channel size is below one
            if (vc.members.size < 1) { 
              //delete it from the map
              intmap.delete(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`); 
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    if (oldState.channelID !== newState.channelID) {
      ...
      if(newState.channelID===config.int) 
      //make a new channel
      ...
      if (intmap.get(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`)) {
        ...
        if (vc.members.size < 1) { 
          ...
          intmap.delete(`tempvoicechannel_${oldState.guild.id}_${oldState.channelID}`); 
         //log it 
          console.log(" :: " + oldState.member.user.username + "#" + oldState.member.user.discriminator + " :: Deleted INT Room")
        //delete the room
          return vc.delete(); 
      }
      else {
    ...
    async function jointocreatechannel(user) {
      //log it 
      client.channels.cache.get(`852786376973877258`).send("Ένας <@&829649546950475796> να εξυπηρετήσει τον/την " + user.member.user.username + "#" + user.member.user.discriminator + " στο 🔉𝐈𝐧𝐭𝐞𝐫𝐯𝐢𝐞𝐰")
      console.log(" :: " + user.member.user.username + "#" + user.member.user.discriminator + " :: Created a INT Room")
      //user.member.user.send("This can be used to message the member that a new room was created")
      await user.guild.channels.create(`🔉𝐈𝐧𝐭𝐞𝐫𝐯𝐢𝐞𝐰`, {
        type: 'voice',
        ...
        intmap.set(`tempvoicechannel_${vc.guild.id}_${vc.id}`, vc.id);
